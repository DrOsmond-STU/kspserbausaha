    ...
    try {
      ...
      kosongkan(isi).append(
        ...
        panelTabel(`Kinerja per Unit Usaha — Tahun ${tahun}`, tabel([
          ...
          { judul: 'Pendapatan', angka: true, render: (u) => rp(u.pendapatan) },
          { judul: 'HPP', angka: true, render: (u) => rp(u.hpp) },
          { judul: 'Laba Kotor', angka: true, render: (u) => rp(u.laba_kotor) },
          { judul: 'Beban', angka: true, render: (u) => rp(u.beban) },
          { judul: 'SHU', angka: true, render: (u) => el(u.shu >= 0 ? 'strong.pos' : 'strong.neg', rp(u.shu)) },
